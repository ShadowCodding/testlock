...
<html lang="en">
  <head>
    ...
    <title>Edit Announcements - <?php echo @$language["SystemName"];?></title>
          ...
          <div class="content-wrapper">
            ...
            <div class="row">
              <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">Edit Announcements - <?= $id?></h4>
                    <form class="forms-sample" action="/announcements/edit/<?= $id?>" method="POST">
                      <?php
                        $query = $db->query("SELECT * FROM announcements WHERE id='$id'", PDO::FETCH_ASSOC);
                        if ($query->rowCount()){
                            foreach( $query as $row ){?>
                      <div class="form-group">
                        <label>Title</label>
                        <input type="text" class="form-control" value="<?= $row["title"]?>" style="color:white;" name="title" id="exampleInputUsername1" placeholder="">
                      </div>
                      
                      <div class="form-group">
                        <label>Data</label>
                        <textarea class="form-control" style="color:white;" name="data" id="exampleFormControlTextarea1" rows="15"><?= $row["data"]?></textarea>
                      </div>

                      <div class="form-group">
                        <label>Status</label>
                        <select class="form-control" name="status" style="width:100%">
                          <option value="false" <?php if ($row["public"] == "false"){echo 'selected';} ?>>Private</option>
                          <option value="true" <?php if ($row["public"] == "true"){echo 'selected';} ?>>Public</option>
                        </select>
                      </div>
