...
<html lang="en">
  <head>
    ...
    <title>Edit License - <?php echo @$language["SystemName"];?></title>
          ...
          <div class="content-wrapper">
            ...
            <div class="row">
              <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">Edit Script - <?= $id?></h4>
                    <form class="forms-sample" action="/user/edit/license/<?= $id?>" method="POST">
                      <?php
                        $query = $db->query("SELECT * FROM licenses WHERE id='$id'", PDO::FETCH_ASSOC);
                        if ($query->rowCount()){
                            foreach( $query as $row ){?>
                      <div class="form-group">
                        <label>Name</label>
                        <input type="text" class="form-control" value="<?= $row["name"]?>" style="color:white;" name="name" id="exampleInputUsername1" placeholder="my bobba script">
                      </div>
                      
                      <div class="form-group">
                        <label>IP</label>
                        <input type="text" class="form-control" style="color:white;" value="<?= $row["ip"]?>" name="ip" id="exampleInputUsername1" placeholder="127.0.0.1">
                      </div>

                      <div class="form-group">
                        <label for="example-date-input"><?php echo @$language["userDeadline"];?></label>
                        <div class="input-group">
                          <div class="input-group-prepend">
                            <span class="input-group-text"><i class="fad fa-calendar-alt"></i></span>
                          </div>
                          <input class="form-control" style="color:white;" name="deadline" value="<?= $row["deadline"]?>" style="line-height: 20px; color:#6c7293;" type="date" name="date" id="example-date-input" required>
                        </div>
                      </div>

                      <div class="form-group">
                        <label><?php echo @$language["userComment"];?></label>
                        <textarea class="form-control" style="color:white;" name="description" id="exampleFormControlTextarea1" rows="5"><?= $row["variables"]?></textarea>
                      </div>
